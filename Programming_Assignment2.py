while True:
    ...
    while True:
        try:
            a = float(input("Enter Kilometers to convert to miles: "))
            b = 0.621371*a
    ...
    while True:
        try:
            date = input("Enter the year and month to show calender in format YYYY MM. Example 2015 02 ")
    ...
    while True:
        try:
            coeff = input("Enter coefficients a,b,c of quadratic equation ax^2+bx+c. Enter in space separated format. Example: 2 10 1 ")
    ...
    while True:
        try:
            num = input("Enter Two numbers to be swapped ")
    ...
    try:
        y_n = input("Do you want to rerun the program.Type 'y' to rerun and 'n' to exit ").lower()
        if y_n == 'n':
            break
        elif y_n == 'y':
            pass
